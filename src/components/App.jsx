...
export class App extends React.Component {
  state = {
    followers: Number(localStorage.getItem('followers')) || 100500,
  };

  onButtonClick = () => {
    if (this.state.followers === 100500) {
      this.setState({
        followers: this.state.followers + 1,
      });
      localStorage.setItem('followers', this.state.followers + 1);
    } else {
      this.setState({
        followers: this.state.followers - 1,
      });
